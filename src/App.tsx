      ...
      for (const image of newImages) {
        try {
          ...
          if (result && result.length > 0) {
            setImages((prev) =>
              prev.map((img) =>
                img.id === image.id ? { ...img, processedFile: result[0] } : img
              )
            );
          }
        } catch (error) {
      ...
      <nav className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-gray-800">BG</h1>
            {!isIOS && (
          ...
          {images.length === 0 && (
            <div className="flex flex-col justify-center items-start">
              ...
              <p className="text-gray-500">
                Upload your image and let our AI instantly erase the background.
                Ideal for professional portraits, product shots, and more.
              </p>
              <p className="text-sm text-gray-300 mt-4">
                Built with love by Jean Marc BOULOS using Transformers.js | 2025
              </p>
            </div>
          )}
